...
import LeverageABI from '../utils/abi/Leverager.json';
import { LeveragerInterface } from 'src/utils/Leverage';
import { getMerklAPR } from 'src/utils/ServerApi';
...
export type LPLoopETHParamsType = {
  interestRateMode: string;
  amount: string;
  borrowRatio: string;
  loopCount: string;
...
export interface PoolSlice {
  ...
  getLoopApprovedAmount: (args: { token: string; spender: string }) => Promise<ApproveType>;
  loop: (args: Omit<LPLoopParamsType, 'user'>) => PopulatedTransaction;
  loopETH: (args: Omit<LPLoopETHParamsType, 'user'>) => PopulatedTransaction;
  setMerkleAPR: () => Promise<void>;
}
...
> = (set, get) => {
  ...
  return {
        ...
        promises.push(
          poolDataProviderContract
            ...
            .then((reservesResponse) => {
              if (currentChainId !== 81457) return reservesResponse;
                ...
                reservesData: reservesResponse.reservesData.map((r) => {
                  if (r.symbol === 'WETH') {
                    r.liquidityRate = BigNumber.from(r.liquidityRate)
                      .add('30250002509832303042121832') // 3.4% native yield
                      .toString();
                  }

                  if (r.symbol === 'USDB') {
                    r.liquidityRate = BigNumber.from(r.liquidityRate)
                      .add('60250002509832303042121832') // 8% native yield
                      .toString();
                  }
    ...
    getLoopApprovedAmount: async (args: { token: string; spender: string }) => {
      const provider = get().jsonRpcProvider();
      const tokenERC20Service = new ERC20Service(provider);
      const user = get().account;
      const amount = await tokenERC20Service.approvedAmount({
        user,
        token: args.token,
    ...
    loop: (args: Omit<LPLoopParamsType, 'user'>) => {
      const currentAccount = get().account;
      const actionTx: PopulatedTransaction = {};
      const iface: LeveragerInterface = new utils.Interface(LeverageABI.abi) as LeveragerInterface;
      const txData = iface.encodeFunctionData('loop', [
        args.asset,
        args.amount,
    ...
    loopETH: (args: Omit<LPLoopETHParamsType, 'user'>) => {
      const currentAccount = get().account;
      const actionTx: PopulatedTransaction = {};
      const iface: LeveragerInterface = new utils.Interface(LeverageABI.abi) as LeveragerInterface;
      const txData = args.isSupply
        ? iface.encodeFunctionData('loopETH', [
            args.interestRateMode,
            args.borrowRatio,
            args.loopCount,
          ])
        : iface.encodeFunctionData('loopETHFromBorrow', [
            args.interestRateMode,
            args.amount,
            args.borrowRatio,
            args.loopCount,
      ...
      actionTx.data = txData;
      if (args.isSupply) {
        actionTx.value = BigNumber.from(args.amount);
      }
      actionTx.gasLimit = BigNumber.from(
