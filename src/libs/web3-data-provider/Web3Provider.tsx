...
import { IPaymasterTransaction } from 'src/store/paymasterSlice';
...
export type Web3Data = {
  ...
  sendGaslessTx: (txData: IPaymasterTransaction) => Promise<TransactionResponse>;
  addERC20Token: (args: ERC20TokenType) => Promise<boolean>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  signTxData: (unsignedData: string) => Promise<SignatureLike>;
  signData: (unsignedData: string, from: string) => Promise<string>;
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const sendGaslessTx = async (txData: IPaymasterTransaction): Promise<TransactionResponse> => {
    if (provider) {
      const { from, ...data } = txData;
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      const zksProvider = new Web3Provider(window.ethereum);
      const signer = zksProvider.getSigner(from);
  ...
  const signData = async (unsignedData: string, from: string): Promise<string> => {
    if (provider && account) {
      const signer = provider.getSigner(from);
      return await signer.signMessage(unsignedData);
    }
    throw new Error('Error initializing signData');
  };
