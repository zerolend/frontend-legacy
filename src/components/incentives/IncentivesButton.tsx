...
export const IncentivesButton = ({ incentives, symbol, displayBlank }: IncentivesButtonProps) => {
  ...
  const fIncentives = useMemo(() => {
    return (
      incentives?.filter((_r) =>
        currentMarketData.whiteListedIncentives.includes(_r.rewardTokenSymbol.toLowerCase())
      ) || incentives
    );
  }, [incentives]);

  if (!(fIncentives && fIncentives.length > 0)) {
    if (displayBlank) {
  ...
  const isIncentivesInfinity = fIncentives.some(
    (incentive) => incentive.incentiveAPR === 'Infinity'
  );
  const incentivesAPRSum = isIncentivesInfinity
    ? 'Infinity'
    : fIncentives.reduce((aIncentive, bIncentive) => aIncentive + +bIncentive.incentiveAPR, 0);
