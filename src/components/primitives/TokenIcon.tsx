  ...
  return (
    ...
    >
      <object
        ...
        ref={ref}
        id="svg"
        data={`https://zerolend.github.io/token-lists/src/tokens/${symbol.toLowerCase()}.svg`}
      ...
      <g id="Group_29109">
        ...
        {symbol && (
          <image
            x="25"
            y="25"
            href={`https://zerolend.github.io/token-lists/src/tokens/${symbol.toLowerCase()}.svg`}
            width="206"
            height="206"
          />
        )}
 ...
 * TokenIcons are expected to be located at /publichttps://zerolend.github.io/token-lists/src/tokens and lowercase named <symbol>.svg
 * @param param0
 * @returns
 */
function SingleTokenIcon({ symbol, aToken, isBadge, ...rest }: TokenIconProps) {
  return (
    <Icon {...rest} sx={{ display: 'flex', position: 'relative', borderRadius: '50%', ...rest.sx }}>
      {aToken ? (
        ...
        // eslint-disable-next-line
        <img
          src={`https://zerolend.github.io/token-lists/src/${isBadge? 'pools': 'tokens'}/${symbol.toLowerCase()}.svg`}
          width="100%"
          height="100%"
          alt={`${symbol} icon`}
        />
...
export function ExternalTokenIcon({ symbol, logoURI, ...rest }: ExternalTokenIconProps) {
  ...
  return (
    <Icon {...rest} sx={{ display: 'flex', position: 'relative', borderRadius: '50%', ...rest.sx }}>
      <LazyLoad>
        <img
          src={tokenSymbol === 'default' || !logoURI ? 'https://zerolend.github.io/token-lists/src/tokens/default.svg' : logoURI}
          width="100%"
          height="100%"
          alt={`${symbol} icon`}
...
export function MultiTokenIcon({ symbols, badgeSymbol, ...rest }: MultiTokenIconProps) {
  ...
  return (
    <Badge
      badgeContent={
        <SingleTokenIcon symbol={badgeSymbol} isBadge sx={{ border: '1px solid #fff' }} fontSize="small" />
      }
