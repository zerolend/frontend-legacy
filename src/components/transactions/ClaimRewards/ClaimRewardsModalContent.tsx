...
import { DetailsNumberLine, DetailsNumberLineWithSub, TxModalDetails } from '../FlowCommons/TxModalDetails';
...
export const ClaimRewardsModalContent = () => {
  ...
  useEffect(() => {
    ...
    Object.entries(user.calculatedUserIncentives).forEach(([rewardTokenAddress, data]) => {
      if (!currentMarketData.whiteListedIncentives.includes(data.rewardTokenSymbol.toLowerCase())) return;
