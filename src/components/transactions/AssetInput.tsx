  ...
  return (
    <Box>
      ...
      <Box
        sx={(theme) => ({
          p: '8px 12px',
          border: borderDisabled ? '' : `1px solid ${theme.palette.divider}`,
          borderRadius: '6px',
          mb: 1,
        })}
      >
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
          ...
          {!onSelect || assets.length === 1 ? (
            <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
              {!isLpToken(asset.iconSymbol || asset.symbol) && (
                <TokenIcon
                  aToken={asset.aToken}
                  symbol={asset.iconSymbol || asset.symbol}
                  sx={isLpToken(asset.iconSymbol || asset.symbol) ? {} : { mr: 2, ml: 4 }}
                  fontSize={isLpToken(asset.iconSymbol || asset.symbol) ? 'large' : 'inherit'}
                />
              )}
