...
export interface StakeZeroActionProps extends BoxProps {
...
export const LinkReferralAction = ({
  isApproved,
  handleLockAction,
  mainTxState,
  approvalTxState,
}: StakeZeroActionProps) => {
  const { readOnlyModeAddress } = useWeb3Context();

  function getMainParams() {
    if (mainTxState?.loading) return { loading: true, disabled: true, content: 'Staking ZERO' };
    ...
    if (isApproved) return { loading: false, disabled: false, content: 'Stake ZERO' };
