...
import { paymasterAssets } from '../FlowCommons/FeeTokenSelector';
...
export const SupplyModalContent = React.memo(
  ({
    ...
    const [feeTokenAddress, setFeeTokenAddress] = useState<string>(paymasterAssets[0].address);
    const [txHash, setTxHash] = useState<string | undefined>();
    const [isPaymaster, setIsPaymaster] = useState<boolean>(false);

    const paymasterData = useRootStore((state) => {
      return txHash && state.paymasterData[txHash] && state.paymasterData[txHash].data
        ? state.paymasterData[txHash].data
        : undefined;
    });

    const paymasterBalance = useGetTokenBalance(paymasterData?.tokenAddress || '');
    ...
    const amountInUsdNumber = isNaN(Number(amountInUsd)) ? 0 : Number(amountInUsd);
    ...
    // Calculate if enough balance for Gasless txn
    const checkBalanceWithGasless = useMemo(() => {
      if (paymasterData && isPaymaster) {
        const paymasterAssetFee = normalize(
          paymasterData.feeTokenAmount,
          Number(paymasterData.feeTokendecimals)
        );

        if (underlyingAsset === paymasterData.tokenAddress) {
          return Number(amount) + Number(paymasterAssetFee) > Number(walletBalance);
        } else {
          return (
            Number(amount) < Number(walletBalance) &&
            Number(paymasterAssetFee) >
              Number(normalize(paymasterBalance.toString(), Number(paymasterData.feeTokendecimals)))
          );
        }
      } else return false;
    }, [amount, paymasterData, isPaymaster, paymasterBalance]);
    ...
    const supplyActionsProps = {
      ...
      blocked: false,
      decimals: poolReserve.decimals,
      isPaymaster: isPaymaster,
      isBalanceSufficient: checkBalanceWithGasless,
    };
    ...
    return (
      <>
        ...
        <TxModalDetails
          ...
          gaslessPossible={isFeatureEnabled?.gasless(currentMarketData)}
          setFeeTokenAddress={setFeeTokenAddress}
          paymasterData={paymasterData}
          amountInUsd={amountInUsdNumber}
          setPaymasterDisable={setIsPaymaster}
        >
