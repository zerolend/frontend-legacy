...
export interface LockLPActionProps extends BoxProps {
...
export const LockLPAction = ({
  ...
  function getMainParams() {
    if (mainTxState?.loading) return { loading: true, disabled: true, content: loadingText };
    if (approvalTxState?.loading)
      return { loading: true, disabled: true, content: 'Approving ETH/ZERO' };
    if (!isApproved && !approvalTxState?.success)
      return { disabled: false, content: 'Approve ETH/ZERO' };
    if (isApproved) return { loading: false, disabled: false, content: text };
    return { content: text, handleClick: handleLockAction };
  }
