...
export type BridgeTxSuccessViewProps = {
  txHash?: string;
  amount: string;
  symbol: string;
  iconSymbol: string;
...
export const BridgeTxSuccessView = ({
  txHash,
  amount,
  symbol,
  iconSymbol,
  ...
  iconUri,
  outIconUri,
}: BridgeTxSuccessViewProps) => {
  return (
    <BaseSuccessView explorer="https://layerzeroscan.com/tx/" txHash={txHash}>
      <Box
        sx={{
          mt: 2,
          display: 'flex',
      ...
      >
        <Typography>
          <Trans>You&apos;ve successfully bridged tokens.</Trans>
        </Typography>
        <Box
          sx={{
            display: 'flex',
