...
export interface LockedDLPActionProps extends BoxProps {
  amount: string;
...
export const LockedDLPAction = ({
  ...
  text,
  loadingText,
}: LockedDLPActionProps) => {
  const { readOnlyModeAddress } = useWeb3Context();

  function getMainParams() {
    if (!amount || Number(amount) < 0.1) return { disabled: true, content: text };
