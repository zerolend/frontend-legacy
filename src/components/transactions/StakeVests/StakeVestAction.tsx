...
export interface StakeVestActionProps extends BoxProps {
...
export const StakeVestAction = ({
  isApproved,
  handleLockAction,
  mainTxState,
  approvalTxState,
}: StakeVestActionProps) => {
  const { readOnlyModeAddress } = useWeb3Context();

  function getMainParams() {
    if (mainTxState?.loading) return { loading: true, disabled: true, content: 'Staking Vest' };
    ...
    if (isApproved) return { loading: false, disabled: false, content: 'Stake Unvested Tokens' };
    return { content: 'Stake Unvested Tokens', handleClick: handleLockAction };
  }
