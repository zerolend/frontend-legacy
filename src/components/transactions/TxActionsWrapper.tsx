...
export const TxActionsWrapper = ({
  ...
  function getMainParams() {
    ...
    if (isBalance) return { disabled: true, content: <Trans>Insufficient Balance</Trans> };
  ...
  function getApprovalParams() {
    ...
    if (approvalTxState?.loading)
      return {
        loading: true,
        disabled: true,
        content: isStrategy ? <Trans>Approving</Trans> : <Trans>Approving {symbol}...</Trans>,
      };
    ...
    return {
      content: (
        <ApprovalTooltip
          ...
          iconMargin={2}
          color="white"
          text={isStrategy ? <Trans>Approve</Trans> : <Trans>Approve {symbol} to continue</Trans>}
        />
      ),
  ...
  const isPaymasterOn: boolean = useMemo(() => {
    if (!isPaymaster) return false;

    if (paymasterData) return false;
    else return true;
  }, [isPaymaster, paymasterData, feeTokenAddress]);

  const { content, disabled, loading, handleClick } = getMainParams();
  const approvalParams = getApprovalParams();

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', mt: 6, ...sx }} {...rest}>
      ...
      {(!isStrategy || (isStrategy && !approvalParams)) && (
        <Button
          variant="contained"
          disabled={disabled || blocked || readOnlyModeAddress !== undefined || isPaymasterOn}
          onClick={handleClick}
          size={isStrategy ? 'medium' : 'large'}
