...
import { useOdoswapQuote } from 'src/hooks/odosswap/useOdosswapRates';
...
export const SwitchModalContent = ({
  ...
  if (sellRates && switchTxState.success) {
    return (
      <SwitchTxSuccessView
        ...
        outAmount={(
          Number(normalize(sellRates.outAmounts[0], selectedOutputToken.decimals)) *
          (1 - Number(slippage))
        ).toString()}
      />
    );
  ...
  const onSwitchReserves = () => {
    ...
    const toInput = sellRates
      ? normalizeBN(sellRates.outAmounts[0], selectedOutputToken.decimals).toString()
      : '0';
        ...
        <>
          ...
          {sellRates && (
            <>
              <SwitchRates
                rates={sellRates}
                srcToken={selectedInputToken}
                destToken={selectedOutputToken}
              />
            </>
          )}
          {sellRates && user && (
