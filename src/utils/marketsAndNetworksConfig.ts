  ...
  return Array.from(
    Object.keys(marketsData)
      .filter((value) => {
        ...
        // note: disable testnet checks for now

        // If this is a staging environment, or the testnet toggle is on, only everything
        if (STAGING_ENV || ENABLE_TESTNET) return true;
...
export const isFeatureEnabled = {
  ...
  gasless: (data: MarketDataType) => data.enabledFeatures?.isGasless,
};
