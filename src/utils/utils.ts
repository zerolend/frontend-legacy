...
export const calculateLoop = (
  ...
  const result =
    Math.log(-(leverageAmount * (1 - ltv) - initialAmount) / initialAmount) / Math.log(ltv);
  return isDepositLoop ? Math.ceil(result - 1) : Math.ceil(result);
};

export const calculateRealLoopAmount = (
  ltv: number,
  loopCount: number,
  initialAmount: number,
  isDepositLoop: boolean
) => {
  let denominator = Number((1 - ltv).toFixed(2));
  let powValue = Math.pow(ltv, isDepositLoop ? loopCount + 1 : loopCount);
  let numerator = 1 - powValue;
  const result = initialAmount * (numerator / denominator);
  return result;
};
