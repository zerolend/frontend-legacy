...
// import { UserIncentiveDict } from '@aave/math-utils/dist/esm/formatters/incentive';
...
export const AppDataProvider: React.FC = ({ children }) => {
  ...
  if (reserves) {
    formattedPoolReserves.forEach((reserve) => {
      merkleAPR.map((merkleAPR) => {
        merkleAPR.tokens.forEach((token) => {
          if (
            merkleAPR.chainId === 1 &&
            currentMarket === 'proto_mainnet_rwa_v3' &&
            reserve.aIncentivesData &&
            token.symbol.toLowerCase() === reserve.symbol.toLowerCase()
          ) {
            reserve.aIncentivesData.push({
              incentiveAPR: (merkleAPR.apr / 100).toFixed(2),
              rewardTokenAddress: token.address,
              rewardTokenSymbol: token.symbol,
            });
          }
          if (
            merkleAPR.chainId === 324 &&
            reserve.aIncentivesData &&
            token.symbol.toLowerCase() === reserve.symbol.toLowerCase() && merkleAPR.rewards.length > 0
          ) {
            reserve.aIncentivesData.push({
              incentiveAPR: (merkleAPR.apr / 100).toFixed(2),
              rewardTokenAddress: merkleAPR.rewards[0].token.address,
              rewardTokenSymbol: merkleAPR.rewards[0].token.symbol,
            });
          }
        });
      });
      ...
      merkleAPR.map((merkleAPR) => {
        merkleAPR.tokens.forEach((token) => {
          if (
            ...
            token.symbol.toLowerCase() === userReserve.reserve.symbol.toLowerCase()
          ) {
            userReserve.reserve.aIncentivesData.push({
              incentiveAPR: (merkleAPR.apr / 100).toFixed(2),
              rewardTokenAddress: token.address,
              rewardTokenSymbol: token.symbol,
            });
          ...
          if (
            currentMarket === 'proto_zksync_era_v3' &&
            userReserve.reserve.aIncentivesData &&
            token.symbol.toLowerCase() === userReserve.reserve.symbol.toLowerCase() && merkleAPR.rewards.length > 0
          ) {
            userReserve.reserve.aIncentivesData.push({
              incentiveAPR: (merkleAPR.apr / 100).toFixed(2),
              rewardTokenAddress: merkleAPR.rewards[0].token.address,
  ...
  /*const filterUserIncentives = (calculatedUserIncentives: UserIncentiveDict) => {
    const c: UserIncentiveDict = {};
    Object.entries(calculatedUserIncentives).map(([key, data]) => {
      if (key.toLowerCase() === '0x861af65b499ac38fc767547fed9c44bba8515a4f'.toLowerCase() && currentMarket === 'proto_blast_v3' ||
        key.toLowerCase() === '0xc6660c3172B0a86Da5ED1FC126401844eD1bDb23'.toLowerCase() && currentMarket === 'proto_manta_v3'
      ) {
        return;
      } else {
        c[key] = data;
  ...
  return (
    <AppDataContext.Provider
      value={{
        ...
        user: {
          ...user,
          // calculatedUserIncentives: filterUserIncentives(user.calculatedUserIncentives),
