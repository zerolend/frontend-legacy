...
const mainnetParaswap = ParaSwap(1);
export const getParaswap = (chainId: number) => {
  if (1 === chainId) return mainnetParaswap;
  ...
  // if (ChainId.linea === chainId) return lineaParaswap;
  // if (ChainId.manta === chainId) return mantaParaswap;
  throw new Error('chain not supported');
};

export const getFeeClaimerAddress = () => {
  return '0x0F6e98A756A40dD050dC78959f45559F98d3289d';
};
...
const ExactInSwapper = (chainId: number) => {
...
const ExactOutSwapper = (chainId: number) => {
  const paraSwap = getParaswap(chainId);
