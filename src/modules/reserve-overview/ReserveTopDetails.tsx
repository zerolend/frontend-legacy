...
export const ReserveTopDetails = ({ underlyingAsset, reserve }: ReserveTopDetailsProps) => {
  ...
  function checkSymbol(symbol: string, chainId: number) {
    return reserve.symbol === symbol && currentMarketData.chainId === chainId;
  }
  ...
  return (
    <>
      ...
      {checkSymbol('ezETH', 1) && (
        <TopInfoPanelItem
          title={<Trans>Total Renzo Points</Trans>}
          loading={points.isLoading}
          hideIcon
        >
          <FormattedNumber
            value={points.value['totalRenzoPoints_1']}
            symbol="Pts"
            sx={{ color: '#ACE731' }}
            variant={valueTypographyVariant}
            symbolsVariant={symbolsTypographyVariant}
      ...
      {checkSymbol('rsETH', 1) && (
        <TopInfoPanelItem
          title={<Trans>Total Kelp Miles</Trans>}
          loading={points.isLoading}
          hideIcon
        >
          <FormattedNumber
            value={points.value.totalKelpMilesPoints_1}
            symbol="Miles"
            sx={{ color: '#BBE7E8' }}
            variant={valueTypographyVariant}
            symbolsVariant={symbolsTypographyVariant}
      ...
      {checkSymbol('weETH', 1) && (
        <TopInfoPanelItem
          title={<Trans>Total Loyalty Points</Trans>}
          loading={points.isLoading}
          hideIcon
        >
          <FormattedNumber
            value={points.value.totalEtherFiPoints_1}
            symbol="Points"
            variant={valueTypographyVariant}
            symbolsVariant={symbolsTypographyVariant}
            symbolsColor="#A5A8B6"
      ...
      {checkSymbol('pufETH', 1) && (
        <TopInfoPanelItem
          title={<Trans>Total Puffer Points</Trans>}
          loading={points.isLoading}
          hideIcon
        >
          <FormattedNumber
            value={points.value.totalPufferFiPoints_1}
            symbol="Points"
            sx={{ color: '#AAEEEE' }}
            variant={valueTypographyVariant}
            symbolsVariant={symbolsTypographyVariant}
