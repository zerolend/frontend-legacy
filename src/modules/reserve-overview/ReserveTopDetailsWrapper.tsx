...
import { isLpToken, lptokensDetails } from 'src/ui-config/lptokenConfig';
...
export const ReserveTopDetailsWrapper = ({ underlyingAsset, reserve }: ReserveTopDetailsProps) => {
    ...
    return (
      <Box mr={3} sx={{ mr: 3, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {loading ? (
          ...
          <TokenIcon symbol={poolReserve.iconSymbol.toLowerCase()} fontSize="large" />
          /*<img
  ...
  const AddLiquidityLink = () => {
    if (isLpToken(poolReserve.iconSymbol)) {
      return (
        <Link
          href={lptokensDetails[poolReserve.iconSymbol].liquidityPoolLink}
  ...
  return (
    ...
    >
      ...
      {isGho ? (
        <GhoReserveTopDetails />
      ) : (
        <ReserveTopDetails underlyingAsset={underlyingAsset} reserve={reserve} />
      )}
    </TopInfoPanel>
  );
};
