...
// accessors
const getDate = (d: FormattedReserveHistoryItem) => new Date(d.date * 1000);
const getData = (d: FormattedReserveHistoryItem, fieldName: Field) => d[fieldName] * 10;
...
export const ApyGraph = withTooltip<AreaProps, TooltipData>(
  ({
    ...
    const dateScale = useMemo(() => {
      // console.log('Extent:- ', extent(data, getDate) as [Date, Date]);
      return scaleTime({
        range: [0, innerWidth],
    ...
    function findNearestObjectIndex(data: FormattedReserveHistoryItem[], unixTimestamp: number) {
      let nearestIndex = -1;
      let smallestDifference = Infinity;

      for (let i = 0; i < data.length; i++) {
        const currentDifference = Math.abs(data[i].date - unixTimestamp);
    ...
    const handleTooltip = useCallback(
      (event: React.TouchEvent<SVGRectElement> | React.MouseEvent<SVGRectElement>) => {
        const { x: mouseXPosition } = localPoint(event) || { x: 0 };
        const chartXPosition = mouseXPosition - margin.left;

        if (chartXPosition < 0 || chartXPosition > innerWidth) {
          hideTooltip();
          return;
        }

        const dateFromScale = dateScale.invert(chartXPosition);
        const unixTimestamp = Math.floor(dateFromScale.getTime() / 1000);

        const nearestIndex = findNearestObjectIndex(data, unixTimestamp);

        showTooltip({
          tooltipData: data[nearestIndex],
          tooltipLeft: chartXPosition,
        });
      },
      [showTooltip, dateScale, data, margin, innerWidth, hideTooltip]
    );
